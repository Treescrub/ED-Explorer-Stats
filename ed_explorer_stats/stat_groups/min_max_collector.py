...
from ..colors import TYPE, STAT, DEFAULT
...
class MinMaxCollector(collector.Collector):
    ...
    def add_type_info(self, type_name, info):
        ...
        if highest_info == lowest_info:
            self.add_line(f"\tHighest/lowest: {STAT}{highest_formatted}{DEFAULT} (object {highest_object} in system {highest_system})")
        else:
            self.add_line(f"\tHighest: {STAT}{highest_formatted}{DEFAULT} (object {highest_object} in system {highest_system})")
            self.add_line(f"\tLowest: {STAT}{lowest_formatted}{DEFAULT} (object {lowest_object} in system {lowest_system})")
